...
class CarSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # Scraping the data from the current page
        sel = Selector(text=self.driver.page_source)
        car_links = sel.xpath('//div[@class="bama-adlist-container"]//a/@href').extract()
        ...
        # Handle pagination - Check if the next page exists
        next_page = sel.xpath('//a[@class="next"]/@href').get()  # Update the XPath according to the site's next button
        if next_page:
            next_page_url = response.urljoin(next_page)
            # Scrapy's way of handling multiple pages
